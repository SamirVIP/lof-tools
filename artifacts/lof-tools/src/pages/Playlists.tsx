...
interface Video { title: string; video_url: string; thumbnail?: string; }
interface Playlist { playlist_title: string; playlist_url: string; updated: string; videos_count: number; videos: Video[]; }
interface RegionEntry { region: string; channel_url: string; playlists_updated: number; playlists: Playlist[]; }
interface ApiResponse { status: string; fetched_at: string; total_regions_active: number; total_playlists_updated: number; data: RegionEntry[]; }
...
export function Playlists() {
  ...
  const filteredRegions = useMemo(() => {
    ...
    return regions
      ...
      .filter((e) => e.playlists.length > 0 || e.region.toLowerCase().includes(q));
  }, [regions, search]);

  const totalShown = filteredRegions.reduce((s, r) => s + r.playlists.length, 0);
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 border-b border-border pb-4">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Search by region, playlist or video title..."
          ...
          {filteredRegions.map((entry) => (
            <div key={entry.region}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-display uppercase tracking-widest text-primary neon-text-primary">{entry.region}</h3>
                <a href={entry.channel_url} target="_blank" rel="noopener noreferrer"
              ...
              {entry.playlists.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {entry.playlists.map((pl, pIdx) => (
                    <div key={pIdx} className="group border border-border bg-card overflow-hidden hover:border-primary/50 transition-colors duration-300 flex flex-col">
                      {pl.videos.length > 0 && pl.videos[0].thumbnail ? (
                        <div className="relative aspect-video overflow-hidden bg-muted">
                          <img src={pl.videos[0].thumbnail} alt={pl.playlist_title}
                        ...
                        {pl.videos.length > 0 && (
                          <div className="mb-4 space-y-1">
                            {pl.videos.slice(0, 3).map((v, vIdx) => (
                              <a key={vIdx} href={v.video_url} target="_blank" rel="noopener noreferrer"
                            ...
                            {pl.videos.length > 3 && (
                              <p className="text-xs font-mono text-muted-foreground/50 pl-5">+{pl.videos.length - 3} more</p>
                            )}
                          </div>
                        )}
                        <Button variant="outline"
                          className="w-full mt-auto font-mono uppercase text-xs tracking-widest border-border hover:border-primary hover:text-primary bg-background/50 hover:bg-primary/10" asChild>
                          <a href={pl.playlist_url} target="_blank" rel="noopener noreferrer">
                            Open Playlist <ExternalLink className="w-3 h-3 ml-2" />
                          </a>
                        </Button>
                      </div>
                    </div>
